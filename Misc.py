...
def Get_Rid_Of_Quoted_Elements(input_2d_array, quoted_string):
    ...
    Store_Single_Data('Current_Action', 'The phrase: ' + quoted_string + ' is being removed from the table')
...
def Get_Elements_With_Class_Sections(input_2d_array, quoted_section):
    ...
    Store_Single_Data('Current_Action', 'The sections: ' + str(quoted_section) + ' is being isolated in the table')
...
def Check_To_See_If_Seat_Available(Table_Input):
    ...
    Store_Single_Data('Current_Action', 'Finally checking to see if seat is available..')
    Seat_Available_List = []
